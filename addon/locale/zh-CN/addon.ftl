...
service-codex=Codex（ChatGPT 登录）
service-codex-dialog-model=模型预设
service-codex-dialog-customModel=自定义模型（仅选择“自定义”时使用）
service-codex-dialog-reasoningEffort=推理强度
service-codex-dialog-timeoutSeconds=超时（秒）
service-codex-dialog-executablePath=Codex 可执行文件
service-codex-dialog-instructions=附加翻译要求
service-codex-model-fast=GPT-5.4 mini — 极速（推荐）
service-codex-model-quality=GPT-5.6 Sol — 高质量
service-codex-model-balanced=GPT-5.6 Terra — 均衡
service-codex-model-light=GPT-5.6 Luna — 轻量
service-codex-model-custom=自定义模型
service-codex-reasoning-none=无 — 最快（推荐）
service-codex-reasoning-low=低
service-codex-reasoning-medium=中
service-codex-reasoning-high=高
service-codex-reasoning-xhigh=极高
service-codex-executable-placeholder=留空则自动查找官方独立版 codex.exe
service-codex-instructions-placeholder=例如：优先采用计算机体系结构领域的标准术语。
service-codex-validation-timeout=超时必须是 10 到 600 秒之间的数字。
service-codex-validation-customModel=选择自定义模型时，必须填写自定义模型名称。
